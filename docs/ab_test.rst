...
**Click-Through Purchase (CTP)** rate over the current production
baseline.
...
where :math:`\mu` denotes the mean CTP of a group. The test is therefore
...
Primary Metric: Click-Through Purchase (CTP)
--------------------------------------------

CTP is the share of recommendation clicks that lead to a purchase:

.. math::

   \text{CTP} = \frac{\#\,\text{Recommendation Purchases}}{\#\,\text{Recommendation Clicks}}

CTP is the right primary metric for this comparison because it isolates
the quality of the *ranking* from the quality of the *retrieval*. CTR
would measure whether a bundle is attractive enough to be clicked, but
not whether it actually converts. CTP captures exactly the downstream
behaviour the recommender is supposed to drive.
...
- Minimum detectable effect (MDE): **10 % relative increase in CTP**
...
clicks within a store are the units of observation. Standard sample
size formulas have to be inflated to account for the fact that clicks
within the same store are not independent.
...
where :math:`n` is the average number of clicks per store during the
test window and :math:`\rho` is the **intraclass correlation (ICC)** —
the share of variance in click outcomes that is explained by the store
the click came from. As a rule of thumb, click-level outcomes in
e-commerce settings have an ICC in the range of 0.01–0.05.

**Required clicks per group**
...
- :math:`\sigma^2 = p_A (1 - p_A)`, the variance of the binary
  click-outcome for the baseline group
- :math:`\Delta = 0.10 \cdot p_A`, the absolute change implied by an
  MDE of 10 % relative

The baseline CTP :math:`p_A`, the ICC :math:`\rho`, and the average
clicks per store :math:`n` are estimated from the most recent two weeks
of production data before the test starts. Plugging these into the
formula gives the required number of clicks per group, which is then
translated into a test duration based on the historical click rate.
...
Even when CTP improves, the new algorithm can have negative
...
After the test window closes, CTP is computed per store, and the two
...
Algorithm B improves CTP over Algorithm A and justifies the rollout.
